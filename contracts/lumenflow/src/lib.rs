...
use types::{
    BatchPaymentItem, GlobalStats, Merchant, MerchantCategory, MerchantCommitment, MerchantPage,
    MerchantStats, MultisigPayment, PaymentFilter, PaymentOrder, PaymentPage, PaymentRequest,
    PaymentStatus, PaymentSummary, RefundRecord, RefundStatus, SortField, SortOrder,
    StatusFilter, SuspiciousActivityReason,
};
...
impl PaymentProcessingContract {
    ...
    ///
    /// # Security note
    /// This single-step path is susceptible to front-running. For new integrations,
    /// prefer the two-step [`commit_merchant_registration`] + [`reveal_merchant_registration`]
    /// flow documented in `docs/merchant-onboarding.md`.
